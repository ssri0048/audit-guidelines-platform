...
/* call พร้อม auto-retry 1 ครั้งเมื่อโดนตัด (เพดาน retry ไม่เกิน 20000 ที่พิสูจน์แล้วว่า API รับ) */
async function callSafe(key, system, user, maxTokens, fetchFn) {
  let r = await apiCall(key, system, user, maxTokens, fetchFn);
  if (r.stop === 'max_tokens') {
    const bigger = Math.min(maxTokens * 2, 20000);
    console.log(`  ↻ คำตอบโดนตัด (${diag(r)}) — retry ด้วยเพดาน ${bigger}`);
    r = await apiCall(key, system, user, bigger, fetchFn);
    if (r.stop === 'max_tokens') throw new Error('ยังโดนตัดหลัง retry (' + diag(r) + ') — ต้องลดขนาดชิ้นงาน');
  }
  return r;
}
...
async function main(topicInput, outDir, key, fetchFn) {
  ...
  for (const r of skeleton.risks) {
    const cfRes = await callSafe(key, SYS,
      `หัวข้อ: "${topicInput}" | ความเสี่ยง: "${r.name_th}" (ระดับ ${r.level})\nมาตรฐานที่อ้างได้: ${inRegistry.join(' | ')}\n\nสร้าง JSON object: {control_failures:[{id:"CF${String(r.id).replace('R','')}-1"...,name_th,audit_procedures:[{id,name_th,method,std_refs:[{std(จากรายการที่อ้างได้เท่านั้น),clause,title}],evidence_types(array ≥4),derivation:"PROFESSIONAL_SYNTHESIS"}]}]}`,
      6000, fetchFn);
