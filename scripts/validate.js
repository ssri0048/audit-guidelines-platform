...
// [เพิ่ม 2026-07-21 — แยก "ธงที่กำลังค้ำความรู้อยู่" ออกจาก "ธงค้างสะสม"]
// เหตุผล: เดิมธงเกิน SLA ทุกตัวบล็อกทุก merge แม้ไม่มีหัวข้อไหนอ้างถึงเลย
// ผลคือหัวข้อใหม่ที่แหล่งอ้างอิงสะอาดสมบูรณ์ก็ขึ้นเว็บไม่ได้ เพราะติดธงเก่าที่ไม่เกี่ยวกัน
// หลักที่ยึด: สิ่งที่ต้องกันคือ "ความรู้บนเว็บอ้างของที่ยังไม่มีหลักฐาน" — ถ้าไม่มีใครอ้าง ก็ไม่ใช่ความเสี่ยงเฉพาะหน้า
// ⚠️ กติกาที่ยังบล็อกเหมือนเดิมทุกกรณี: G5_COVERAGE_NEW (ratchet), G4 ปีอนาคต, WITHDRAWN/RETIRED, readproof
const backlog = []; // ธงค้างที่ไม่มีใครอ้าง — เป็นบันทึกไว้ตามแก้ ไม่บล็อก
function citedBy(fam) {
  let re; try { re = new RegExp(fam.match, 'i'); } catch (e) { return []; }
  const ids = [];
  for (const t of store.topics) {
    if ((t.applicable_standards || []).some(s => re.test(s))) ids.push(t.id);
  }
  return ids;
}
...
for (const fam of REGISTRY.families) {
  ...
  for (const ed of fam.editions || []) {
    ...
    if (ed.verified === false) {
      ...
      // [ปรับปรุง 2026-07-21] เดิม: errors.push(`[SLA] ... ต้องปิดธงก่อน merge`) — บล็อกทุกกรณี
      // ใหม่: บล็อกเฉพาะเมื่อมีหัวข้อในคลังอ้างมาตรฐานนี้อยู่จริง (ความรู้ที่เผยแพร่แล้วขาดหลักฐาน = ต้องหยุด)
      if (age !== null && age > SLA_DAYS) {
        const used = citedBy(fam);
        if (used.length) {
          errors.push(`[SLA] ${fam.display} v${ed.version} — UNVERIFIED ค้าง ${age} วัน และมีหัวข้ออ้างอยู่ (${used.join(',')}) ต้องปิดธงก่อน merge`);
        } else {
          backlog.push({ ref: `${fam.display} v${ed.version}`, age, todo: (ed.notes || '').slice(0, 70) });
          warnings.push(`[SLA_BACKLOG] ${fam.display} v${ed.version} — UNVERIFIED ค้าง ${age} วัน · ยังไม่มีหัวข้อใดอ้างถึง จึงไม่บล็อกการอนุมัติ แต่ยังอ้างในหัวข้อใหม่ไม่ได้`);
        }
      }
...
if (queue.length) {
  ...
  // [เพิ่ม 2026-07-21] สรุปธงค้างสะสมให้เห็นชัด — ไม่บล็อก แต่ต้องไม่เงียบหาย
  if (backlog.length) {
    const oldest = backlog.reduce((m, b) => Math.max(m, b.age || 0), 0);
    console.log(`\n── บันทึกรายการค้างสะสม (${backlog.length} รายการ · เก่าสุด ${oldest} วัน · ไม่บล็อกการอนุมัติ) ──`);
    backlog.sort((a, b) => (b.age || 0) - (a.age || 0)).forEach(b =>
      console.log(`  🏳️  ${b.ref} — ค้าง ${b.age} วัน${b.todo ? ' · ' + b.todo : ''}`));
    console.log('  ยังไม่มีหัวข้อใดอ้างรายการเหล่านี้ · ถ้าหัวข้อใหม่ไปอ้าง ระบบจะบล็อกทันที (ratchet ยังทำงานเต็มที่)');
  }
}
